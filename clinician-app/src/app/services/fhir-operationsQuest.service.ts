...
export class FhirOperationsService {
  ...
  generateQuestionnaireResponse(formValues, quest: any) {
    ...
    identifier.value = 'WHO_MODUELE_1';
    ...
    quest.item.forEach(elementGroup => {
      ...
      elementGroup.item.forEach(elementItem => {
        const questionnaireResponseItem = scope.generateQuestionnaireResponseItem(elementItem, elementItem.linkId, formValues[elementGroup.linkId][elementItem.linkId]);
  ...
  generateQuestionnaireResponseItem(elementItem, linkId, childItem) {
    ...
    if (childItem) {
      ...
      questionnaireResponseItem.text = elementItem.text;
