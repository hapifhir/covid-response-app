...
export class FhirOperationsService {
  ...
  generateQuestionnaireResponse(formValues,quest?:any)
  {
    if (quest===undefined || quest===null)
      this.generateQuestionnaireResponseBasic(formValues);
    else
      this.generateQuestionnaireResponseFull(formValues,quest);

  }

  generateQuestionnaireResponseFull (formValues,quest:any) {
    const scope = this;
    ...
    quest.item.forEach(elementGroup=>{
      ...
      elementGroup.item.forEach(elementItem=>{
        const questionnaireResponseItem = scope.generateQuestionnaireResponseItemFull(elementItem,elementGroup.linkId + '_' + elementItem.linkId, formValues[elementGroup.linkId][elementItem.linkId]);
  ...
  generateQuestionnaireResponseBasic(formValues) {
    const scope = this;
    const questionnaireResponse = new FHIR.QuestionnaireResponse();
    const identifier = new FHIR.Identifier();
    identifier.value = 'SARS_Questionnaire_Response';
  ...
  generateQuestionnaireResponseItemFull(elementItem,linkId, childItem) {
    let returnItem = null;
    if(childItem) {
      const questionnaireResponseItem = new FHIR.QuestionnaireResponseItem();
      const questionnaireAnswer = new FHIR.Answer();
